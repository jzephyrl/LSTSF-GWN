...
#构建基础网络
def build_model(inputs,n_his,Kt, blocks, keep_prob,x_b1,x_b2,reduction_ratio):
    '''
    ...
    for i, channels in enumerate(blocks):
        x=st_conv_block(x,Kt,channels,i,keep_prob,x_b1,x_b2,reduction_ratio,act_func='GLU')
        Ko -= 2 * (Kt - 1)
    ...
    tf.add_to_collection(name='copy_loss',value=tf.reduce_sum(tf.losses.huber_loss(inputs[:, n_his:n_his + 1, :, :],inputs[:, n_his - 1:n_his, :, :])))
    train_loss = tf.reduce_sum(tf.losses.huber_loss(inputs[:,n_his:n_his+1,:,:],y))
    single_pred = y[:, 0, :, :]
