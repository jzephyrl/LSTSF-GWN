...
def multi_pred(sess, y_pred, seq, batch_size, n,n_his, n_pred, step_idx,threshold,sparse_ness,s,weight_normalize,adj_scale,dynamic_batch=True):
    '''
    ...
    for i in gen_batch(seq, min(batch_size, len(seq)), dynamic_batch=dynamic_batch):
        ...
        for j in range(n_pred):
            pred = sess.run(y_pred,feed_dict={'data_input:0': test_seq, 'keep_prob:0': 1.0,'x_b1:0':LW[0][:,:],'x_b2:0':LW[1][:,:]})
...
def model_inference(sess, pred, inputs, batch_size, n,n_his, n_pred, step_idx, min_va_val, min_val,threshold,sparse_ness,s,weight_normalize,adj_scale):
    '''
    ...
    y_val, len_val = multi_pred(sess, pred, x_val, batch_size,n, n_his, n_pred, step_idx,threshold,sparse_ness,s,weight_normalize,adj_scale)
    ...
    if sum(chks):
        ...
        y_pred, len_pred = multi_pred(sess, pred, x_test, batch_size,n, n_his, n_pred, step_idx,threshold,sparse_ness,s,weight_normalize,adj_scale)
        evl_pred = evaluation(x_test[0:len_pred, step_idx + n_his, :, :], y_pred, x_stats)
        min_val = evl_pred
    return min_va_val, min_val


def model_test(inputs, batch_size,n, n_his, n_pred,inf_mode,threshold,sparse_ness,s,weight_normalize,adj_scale,load_path):
    '''
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        y_test, len_test = multi_pred(test_sess, pred, x_test, batch_size,n, n_his, n_pred, step_idx,threshold,sparse_ness,s,weight_normalize,adj_scale)
